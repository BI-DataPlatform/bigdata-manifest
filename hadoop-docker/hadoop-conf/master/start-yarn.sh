...
echo "Starting NodeManagers on all worker nodes..."
for worker in $(cat $HADOOP_CONF_DIR/workers); do
  echo "Starting NodeManager on $worker"
  ssh $HADOOP_SSH_OPTS $worker "bash -c 'export HADOOP_CONF_DIR=$HADOOP_CONF_DIR; \
        export HADOOP_YARN_HOME=$HADOOP_YARN_HOME; \
        ${HADOOP_YARN_HOME}/bin/yarn --config ${HADOOP_CONF_DIR} nodemanager'" &
  if [ $? -ne 0 ]; then
    echo "Failed to start NodeManager on $worker"
